...
export default function BrandPage() {
  ...
  useEffect(() => {
    async function fetchSnacks() {
      ...
      const filteredSnacks = (data || []).filter((snack) => makeSlug(snack.brand) === brandSlug);
    ...
    <main className="min-h-screen bg-black px-10 py-16 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex items-end justify-between">
          <div>
            <h1 className="mb-1 text-4xl font-black">{formatTitle(brandSlug)}</h1>
            <p className="text-white/50">Showing all snacks from this brand.</p>
          </div>
          <select
            value={sortBy}
            ...
            {sortedSnacks.map((snack) => (
              ...
              >
                <Link href={`/${snack.slug}`} className="flex flex-col flex-1">
                  ...
                  <p className="mt-1 text-sm text-white/45">{snack.category}</p>
